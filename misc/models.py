...
def input_space_3d(image_depth, image_height, image_width):
    ...
    mgrid = torch.stack(torch.meshgrid(*tensors), dim=-1)  # [d, h, w, 3]

    return mgrid


def input_space_3d_padding(
    image_depth, image_height, image_width, pad_depth, pad_height, pad_width
):
    """Generate 3D grid coordinate with padding,
    original coordinate in [-1, 1],
    padded coordinates in [-1-pad_n*\delta, 1+pad_n*\delta]

    Return: 4D tensor of shape [D+pad_d, H+pad_h, W+pad_w, 3]
    """
    d_start = -1 - pad_depth / (image_depth - 1)
    d_end = 1 + pad_depth / (image_depth - 1)
    h_start = -1 - pad_height / (image_height - 1)
    h_end = 1 + pad_height / (image_height - 1)
    w_start = -1 - pad_width / (image_width - 1)
    w_end = 1 + pad_width / (image_width - 1)
    tensors = [
        torch.linspace(d_start, d_end, steps=image_depth + pad_depth),
        torch.linspace(h_start, h_end, steps=image_height + pad_height),
        torch.linspace(w_start, w_end, steps=image_width + pad_width),
    ]
    mgrid = torch.stack(torch.meshgrid(*tensors), dim=-1)  # [d+pd, h+ph, w+pw, 3]
...
class PhysicsInformedSphericalEmbedding(nn.Module):
    # Almost the same function as PhysicsInformedSphericalEmbedding,
    # just with a little bit different in the fourier mapping matrix
    # as depicted on `construct_fourier_mapping` function
    def __init__(
        self,
        theta_degree,
        phi_degree,
        ...
        super(PhysicsInformedSphericalEmbedding, self).__init__()
        self.theta = torch.arange(0, 180, theta_degree) * np.pi / 180
        self.phi = torch.arange(phi_degree, 180, phi_degree) * np.pi / 180
        self.encoding_length = int(np.floor(L))
        self.theta_num = len(self.theta)
        self.phi_num = len(self.phi)
        # self.out_channels = (self.theta_num * self.phi_num + 1) * (2 * self.encoding_length) + 3
        self.out_channels = (self.theta_num * self.phi_num + 1) * (
            2 * self.encoding_length
        )
        self.fz_cut = fz_cut
        self.fy_cut = fy_cut
        self.fx_cut = fx_cut
...
class Seminerf(nn.Module):  # Spherical encoded fluorescence density field
    def __init__(
        ...
        skips=[4],
        out_channels=1,
        encoding_mode="spherical",
        radial_encoding_angle=30,
        radial_encoding_depth=7,
        ...
        if encoding_mode == "spherical":
            ...
            self.embedding = PhysicsInformedSphericalEmbedding(
                radial_encoding_angle,
                zenith_encoding_angle,
                radial_encoding_depth,
                device=device,
