...
class PhysicsParamsManager(object):
    def __init__(self, args):
        ...
        self.dy = args.psf_dy
        self.dx = args.psf_dx
        self.NA = args.n_detection  # Numerical Aperture
        self.ni = args.n_immersion  # Emission light wave length (um)
        self.ns = args.n_obj  # Refractive index of the object
        self.wave_length = (
            args.emission_wavelength
    ...
    def ENC_params(self, coord_shape: tuple) -> dict:
        ...
        dfx = 1 / (w * self.dx)
        params = {
            # "fz_cut": np.min((self.z_cut_freq / dfz, d//2)),
            # "fy_cut": np.min((self.r_cut_freq / dfy, h//2)),
            # "fx_cut": np.min((self.r_cut_freq / dfx, w//2)),
