...
class FDMAE_Loss(torch.nn.Module):
    ...
    def __init__(self, shape: tuple, phys_params: dict, device="cpu"):
        super(FDMAE_Loss, self).__init__()
        ...
        if len(shape) == 2:
            h, w = shape
            # self.window = window_func(h).unsqueeze(1) * window_func(w).unsqueeze(0)
            self.window = (
            ...
            d, h, w = shape
            # self.window = (
            #     window_func(d).unsqueeze(1).unsqueeze(1) * window_func(h).unsqueeze(1) * window_func(w).unsqueeze(0)
            # )
            self.window = (
            ...
            raise ValueError("Invalid shape for FDMAE_Loss")
