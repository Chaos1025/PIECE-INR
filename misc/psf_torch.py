...
class PsfGenerator3D:
    ...
    def __init__(self, psf_shape, units, lam_detection, n, na_detection, n_threads=4):
        ...
        Arguments:
            -- psf_shape: shape of the PSF volumn, (Nz, Ny, Nx)
            -- units: sampling interval along each axis, (dz, dy, dx)
            -- lam_detection: wavelength of the detection light
            -- n: refractive index of the medium
            #? what medium?
            -- na_detection: numerical aperture of the detection light
        '''
        
        psf_shape = tuple(psf_shape) # sampling length along each axis
        units = tuple(units) # voxel size along each axis, or sampling interval
        print("Generate PSF with shape: ", psf_shape, " and units: ", units, ", using angular spectrum method.")
        ...
        #* Discret Fourier Frequence
        kx = torch.fft.fftfreq(self.Nx, self.dx).type(dtype).cuda(0) # like [0.000, 0.025, 0.050, 0.075, -0.100, -0.075, -0.050, -0.025]
        ky = torch.fft.fftfreq(self.Ny, self.dy).type(dtype).cuda(0)
        #* Spatial Sampling
        z = self.dz * (torch.arange(self.Nz) - self.Nz // 2) #? Why?
        z = z.type(dtype).cuda(0)

        self.KZ3, self.KY3, self.KX3 = torch.meshgrid(z, ky, kx, indexing="ij") # [Nz, Ny, Nx]
        # self.KZ3, self.KY3, self.KX3 = torch.meshgrid(z, ky, kx)
        KR3 = torch.sqrt(self.KX3 ** 2 + self.KY3 ** 2) # radial coordinate in voxel space, [Nz, Ny, Nx]

        # the cutoff frequency in fourier domain (coherent cutoff)
        self.kcut = 1. * na_detection / self.lam_detection
        self.kmask3 = (KR3 <= self.kcut).type(dtype).cuda(0) # similar to aperture

        H = torch.sqrt(1. * self.n ** 2 - KR3 ** 2 * lam_detection ** 2).type(dtype).cuda(0) #? What's this?

        self._H = H

        out_ind = torch.isnan(H) # where k^2 * lam^2 > n^2
        self.kprop = torch.exp(-2.j * np.pi * self.KZ3 / lam_detection * H) # the wave vertor in propagation direction
        self.kprop[out_ind] = 0.

        self.kbase = self.kmask3 * self.kprop

        KY2, KX2 = torch.meshgrid(ky, kx, indexing="ij") # 2D coord, [Ny, Nx]
    ...
    def zernike_polynomial(self, idx, normalized = True):
        '''Calculate Zernike polynomial with the order of 'idx'
    ...
    def masked_phase_array(self, phi, normalized=True, piston_tip_tilt=False):
        ''' Generate phase aberration with the 'phi' as zernike coefficients, [Ny, Nx]
