...
class Block_Scheduler(nn.Module):
    ...
    def __init__(
        ...
        self.stack = (
            whole_stack.astype(np.float32) / whole_stack.max()
        )
        self.mask_mode = mask_mode
        self.axial_view = (
            whole_stack.shape[0] if axial_view > whole_stack.shape[0] else axial_view
        )
    ...
    def get_global_mask(self):
        ...
        for i in range(self.height_block_num - 1): # height, row
            global_mask[
                :,
                (i + 1) * self.step_size : (i + 1) * self.step_size
        ...
        for j in range(self.width_block_num - 1): # width, column
            global_mask[
                :,
                :,
                (j + 1) * self.step_size : (j + 1) * self.step_size
                + self.lateral_overlap,
            ] *= (
                1 / 2
            )
    ...
    def get_single_mask(self, row_index: int, col_index: int):
        ...
        if self.mask_mode == "smooth":
            if not pos[3]:  # not top edge
                mask[: self.lateral_overlap, :] *= np.linspace(
                    0, 1, self.lateral_overlap
                )[:, None]
            if not pos[2]:  # not bottom edge
                mask[-self.lateral_overlap :, :] *= np.linspace(
                    1, 0, self.lateral_overlap
                )[:, None]
    ...
    def feedback_recovered_block(
        self, row_index: int, col_index: int, recovered_block: np.array
    ):
...
class Global_Block_Scheduler:
    def __init__(
        ...
        self.coord_range = [
            self.sample_range[i] + self.padding_array[i] for i in range(3)
        ]
        self.lateral_view = lateral_view
        self.lateral_overlap = lateral_overlap
        self.step_size = self.lateral_view - self.lateral_overlap
